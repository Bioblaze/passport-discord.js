...
class Strategy extends OAuth2Strategy {
  ...
  userProfile(token, done) {
    this._oauth2.get("https://discordapp.com/api/users/@me", token, function (err, body, res) {
      ...
      Promise.all([
        this.checkScope("connections", token),
        this.checkScope("guilds", token)
  ...
  checkScope(scope, token) {
    return new Promise(function(res, rej) {
      if (this._options.scope.indexOf(scope) > -1) {
        this._oauth2.get(`https://discordapp.com/api/users/@me/${scope}`, token, function (err, body, _res) {
          if (err) {
